class KFProjectileStickHelper_Zap extends KFProjectileStickHelper;

var() float MinDistanceFromPlayer;
var() float DampingFactor;
var() bool 	bNotAltFire;
...
*/
		...
		if( WorldInfo.NetMode != NM_DedicatedServer )
		{
			`ImpactEffectManager.PlayImpactEffects(HitLocation, Instigator,, KFProj_Rocket_Zap(Outer).ImpactEffects);
		}
	}
}
...
simulated function StickToActor(Actor StickTo, PrimitiveComponent HitComp, int BoneIdx, optional bool bCalculateRelativeLocRot)
{
	...
	if (bNotAltFire)
	{
		KFProj_Rocket_Zap(Outer).NotifyStick();
	}
}
...
defaultproperties
{
    MinDistanceFromPlayer=3500.0f //1500.0f
    DampingFactor=0.02
	bNotAltFire=true //false
}